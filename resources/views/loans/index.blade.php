...
@section('content')
    <div class="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 sm:mb-8">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Daftar Peminjaman</h1>
        ...
        <div
            ...
            <form method="GET" action="{{ route('loans.index') }}"
                class="flex flex-col sm:flex-row sm:flex-wrap gap-3 sm:gap-4">
                        ...
                        @forelse($loans as $loan)
                            <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition">
                                ...
                                <td class="px-6 py-4 text-sm">
                                    <span
                                        class="{{ $loan->isOverdue() ? 'text-pink-600 font-medium' : 'text-gray-900 dark:text-gray-300' }}">
                                ...
                                <td class="px-6 py-4">
                                    @if($loan->status === 'active')
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-teal-100 text-teal-700 dark:bg-teal-900/50 dark:text-teal-300">
                                            Aktif
                                        </span>
                                    @else
                                        <span
                                ...
                                <td class="px-6 py-4 text-sm">
                                    @if($loan->fine_amount > 0)
                                        <span
                                            class="text-pink-600 dark:text-pink-400 font-medium">Rp{{ number_format($loan->fine_amount, 0, ',', '.') }}</span>
                                    @else
