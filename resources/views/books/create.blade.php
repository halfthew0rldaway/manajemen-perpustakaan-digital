...
@section('content')
    <div class="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="max-w-3xl mx-auto">
            <!-- Header -->
            <div class="mb-8">
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Tambah Buku Baru</h1>
                <p class="text-gray-600">Masukkan informasi buku baru ke dalam koleksi perpustakaan.</p>
            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="title" id="title" class="form-input" value="{{ old('title') }}"
                                    required placeholder="Masukkan judul buku lengkap">
                                @error('title')
                            ...
                            <div>
                                ...
                                <input type="text" name="isbn" id="isbn" class="form-input" value="{{ old('isbn') }}"
                                    placeholder="Contoh: 978-602-03-0388-3">
                                @error('isbn')
                                    <p class="mt-1 text-sm text-pink-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="publication_year" id="publication_year" class="form-input"
                                    value="{{ old('publication_year') }}" placeholder="Tahun (YYYY)" min="1900"
                                    max="{{ date('Y') }}">
                            ...
                            <div>
                                ...
                                <input type="number" name="stock" id="stock" class="form-input"
                                    value="{{ old('stock', 1) }}" required min="1" placeholder="Jumlah buku tersedia">
                                @error('stock')
                                    <p class="mt-1 text-sm text-pink-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="description" id="description" rows="4" class="form-textarea"
                                    placeholder="Ringkasan atau sinopsis buku">{{ old('description') }}</textarea>
                                @error('description')
                                    <p class="mt-1 text-sm text-pink-600">{{ $message }}</p>
                                @enderror
                            </div>
